...
file_onnx = "/home/user/Documents/tensorrt_yolo26/build/output_onnx.txt"
file_engine = "/home/user/Documents/tensorrt_yolo26/build/output_engine.txt"
...
print(f"Difference GAP: {min_difference} - {max_difference} ----- MEAN: {mean_difference}")
print(f"ONNX GAP: {np.min(numpy_onnx)} - {np.max(numpy_onnx)} ----- MEAN: {np.mean(numpy_onnx)}")
print(f"ENGINE GAP: {np.min(numpy_engine)} - {np.max(numpy_engine)} ----- MEAN: {np.mean(numpy_engine)}")