...

{
    ...
    *******************************************************************************************************/
    ...
    nvinfer1::IConcatenationLayer* concat23_1 = network->addConcatenation(inputTensors23_1, 3);
    concat23_1->setAxis(2);
    nvinfer1::IActivationLayer* sigmoid23 = network->addActivation(
            *concat23_1->getOutput(0),
    ...
    nvinfer1::IConcatenationLayer* concat23 = network->addConcatenation(inputTensors23, 3);
    concat23->setAxis(2);

    /////////////////////////////////////////////////////

    nvinfer1::ISliceLayer* slice23_1 =
            network->addSlice(*concat23->getOutput(0), nvinfer1::Dims3{0, 0, 0},
                              nvinfer1::Dims3{concat23->getOutput(0)->getDimensions().d[0],
                                              concat23->getOutput(0)->getDimensions().d[1] / 2,
                                              concat23->getOutput(0)->getDimensions().d[2]},
                              nvinfer1::Dims3{1, 1, 1});
    nvinfer1::ISliceLayer* slice23 = network->addSlice(
            *concat23->getOutput(0), nvinfer1::Dims3{0, concat23->getOutput(0)->getDimensions().d[1] / 2, 0},
            nvinfer1::Dims3{concat23->getOutput(0)->getDimensions().d[0],
                            concat23->getOutput(0)->getDimensions().d[1] / 2,
                            concat23->getOutput(0)->getDimensions().d[2]},
            nvinfer1::Dims3{1, 1, 1});

    // TODO: MOVE BELOW GRID CODE TO A SEPARATE FUNCTION
    const int anchor_num = concat23->getOutput(0)->getDimensions().d[2];
    ...
    nvinfer1::IConcatenationLayer* conv23_concat_2 = network->addConcatenation(tensor23, 2);
    ...
    nvinfer1::IElementWiseLayer* mul23_2 = network->addElementWise(
            *conv23_concat_2->getOutput(0), *constant_stride->getOutput(0), nvinfer1::ElementWiseOperation::kPROD);

    ///////////////////////////////////////////////////////////

    nvinfer1::IConcatenationLayer* concat23_3 = network->addConcatenation(
            std::array<nvinfer1::ITensor*, 2>{mul23_2->getOutput(0), sigmoid23->getOutput(0)}.data(), 2);
    concat23_3->setAxis(1);

    nvinfer1::IShuffleLayer* transpose = network->addShuffle(*concat23_3->getOutput(0));
    ...
    nvinfer1::IPluginV2Layer* yolo =
            addYoloLayer(network, *transpose->getOutput(0), strides, fm_sizes, stridesLength, false, false, false, anchor_num);
    assert(yolo);
